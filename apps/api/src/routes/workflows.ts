 ...
 */
export function workflowProxyRoutes(
  db: Db,
  ...
  app.post("/clients/:clientId/workflows/plan-generation", async (c) => {
    ...
    return proxy(c, config, "/workflows/plan-generation", {
      method: "POST",
      body,
    });
  });
...
async function proxy(
  ...
  try {
    ...
    return errorResponse(
      c,
      502,
      "workflow_api_unreachable",
      "the workflow-start API could not be reached",
    );
  }
  const body: unknown = await upstream.json().catch(() => null);
  if (body === null) {
    console.error("[api] workflow proxy bad response", {
      path,
      method: init.method,
    ...
    return errorResponse(
      c,
      502,
      "workflow_api_bad_response",
      "the workflow-start API returned an invalid response",
    );
  }
  if (init.method === "POST") {
    console.info("[api] workflow proxy", {
      path,
      status: upstream.status,
      body,
    });
