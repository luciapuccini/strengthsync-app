...
export function planRoutes(db: Db): Hono {
  ...
  app.get('/clients/:clientId/plans/active', async (c) => {
    ...
    if (!plan) {
      return errorResponse(c, 404, 'active_plan_not_found', `client ${clientId} has no active plan`)
    }
    return c.json({ plan })
  })
  ...
  app.get('/clients/:clientId/plans/:planId', async (c) => {
    ...
    const plan = await getPlan(db, clientId, planId)
    if (!plan) {
      return errorResponse(c, 404, 'plan_not_found', `plan ${planId} not found`)
    }
    return c.json({ plan })
  })
