...
its short form (`api`, `ui`, `domain`, `db`).
...
`apps/workflows` and `services/agent` packages); re-run `pnpm ts:diagnostics`
for current numbers.
...
- Own source is small (8.3k lines across 4 packages); the overwhelming
...
- 4 composite packages is comfortably inside the empirically reasonable
...
- Each package's `package.json` `exports` map points at specific entry
  ...
  (`services/db`, `apps/api`), it is a small, curated
...
- `types: []` is already set on every package that doesn't need ambient
  globals (`services/domain`), preventing ambient
