...
async function requireClientAndProfile(db: Db, clientId: string) {
  ...
  if (!profile) {
    throw new RepoError(
      "not_found",
      "profile_not_found",
      `client ${clientId} has no profile`,
    );
  }
...
export async function getWeeklyContext(
  ...
  const { client, profile, activePlan } = await requireClientContext(
    db,
    clientId,
  );
...
export async function completeWeek(
  ...
  throw new RepoError(
    "conflict",
    "week_complete_conflict",
    `week ${weekId} could not be completed`,
  );
}
...
export async function createNextWeek(
  ...
  if (!plan) {
    throw new RepoError(
      "not_found",
      "plan_not_found",
      `plan ${previous.plan_id} not found`,
    );
  }
  ...
  const completedWeeks = await db
    ...
    .where(
      and(
        ...
        eq(weeks.plan_id, activePlan.id),
        eq(weeks.status, "completed"),
      ),
    )
    .orderBy(asc(weeks.start_date));
  const finalWeek = completedWeeks.find(
    (week) => week.week_index === activePlan.total_weeks,
  );
  if (!finalWeek) {
    throw new RepoError(
      "validation",
