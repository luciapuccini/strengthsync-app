...
const jsonRecord = () => text({ mode: 'json' }).$type<Record<string, unknown>>()
...
export const plans = sqliteTable(
  ...
  (t) => [
    ...
    uniqueIndex('plans_one_active_per_client').on(t.client_id).where(sql`status = 'active'`),
...
export const weeks = sqliteTable(
  ...
  (t) => [
    ...
    uniqueIndex('weeks_one_in_flight_per_client').on(t.client_id).where(sql`status = 'in_flight'`),
